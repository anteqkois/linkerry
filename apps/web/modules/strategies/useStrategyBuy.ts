...
import { strategyBuySchema } from './validations'


export const useStrategyBuy = () => {
  const [isLoading, setIsLoading] = useState(false)

  const form = useForm<z.infer<typeof strategyBuySchema>>({
    resolver: zodResolver(strategyBuySchema),
    defaultValues: {
  ...
  const onSubmit = async (values: z.infer<typeof strategyBuySchema>) => {
    console.log(values)
    setIsLoading(true)
    try {
