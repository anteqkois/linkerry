...
const setCookie = <S>(key: string, value: S, options?: Options) => {
  try {
    ...
    const expires = new Date(Date.now() + optionsWithDefaults.expireDays * 864e5).toUTCString()

    // from exxpressJS, to be set in teh same way
    const val = typeof value === 'object' ? String(`j:${JSON.stringify(value)}`) : String(value)

    const cookie = `${key}=${encodeURIComponent(val)}; expires=${expires} ${stringifyOptions(optionsWithDefaults)}`
...
function useCookie<S>(key: string, initialValue: S | (() => S), options?: Options): [S, Dispatch<SetStateAction<S>>]

function useCookie<S = undefined>(key: string): [S | undefined, (value: S, options?: Options) => void]
