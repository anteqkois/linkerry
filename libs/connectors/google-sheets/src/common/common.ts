import { AuthenticationType, HttpMethod, HttpRequest, getAccessTokenOrThrow, httpClient } from '@linkerry/connectors-common'
import { OAuth2PropertyValue, Property } from '@linkerry/connectors-framework'
...
export const googleSheetsCommon = {
  ...
  spreadsheet_id: Property.DynamicDropdown({
  ...
  sheet_id: Property.DynamicDropdown({
    displayName: 'Sheet',
    description: 'Sheet',
    required: true,
  ...
  columnName: Property.DynamicDropdown<boolean, string>({
...
export async function findSheetName(access_token: string, spreadsheetId: string, sheetId: number) {
