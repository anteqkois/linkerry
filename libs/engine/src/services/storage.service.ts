import { Store, StoreScope } from '@linkerry/connectors-framework'
import { DeletStoreEntryRequest, Id, PutStoreEntryRequest, StoreEntry } from '@linkerry/shared'
...
export function createContextStore({ prefix, flowId, workerToken }: { prefix: string; flowId: Id; workerToken: string }): Store {
	return {
...
function createKey(prefix: string, scope: StoreScope, flowId: Id, key: string): string {
	switch (scope) {
