...
export class VariableService {
	...
	private async resolveInternally(unresolvedInput: any, valuesMap: any, logs: boolean): Promise<any> {
		...
		if (Array.isArray(unresolvedInput)) {
			for (let i = 0; i < unresolvedInput.length; ++i) {
				unresolvedInput[i] = await this.resolveInternally(unresolvedInput[i], valuesMap, logs)
			}
		} else if (typeof unresolvedInput === 'object') {
			const entries = Object.entries(unresolvedInput)
			for (const [key, value] of entries) {
				unresolvedInput[key] = await this.resolveInternally(value, valuesMap, logs)
			}
		}
	...
	async resolve<T = unknown>(params: {
		...
		const resolvedInput = await this.resolveInternally(JSON.parse(JSON.stringify(unresolvedInput)), executionState.currentState, false)
		const censoredInput = await this.resolveInternally(JSON.parse(JSON.stringify(unresolvedInput)), executionState.currentState, true)
		return {
			resolvedInput,
			censoredInput,
		}
