...
export class AppConnectionsService {
  ...
  private async _engineValidateAuth({
    ...
    if (!validateAuthResult.valid) {
      // if (typeof validateAuthResult === 'object' && Object.hasOwn(validateAuthResult, 'error'))
      if (typeof validateAuthResult === 'object' && 'error' in validateAuthResult)
        throw new CustomError(validateAuthResult.error, ErrorCode.INVALID_APP_CONNECTION, validateAuthResult)
      else
        throw new CustomError(
          'Can not verify your connection. There is a problem with our validation service. Please inform our IT tema.',
