...
export class FlowRunsWebSocketService implements OnGatewayConnection, OnGatewayDisconnect {
  ...
  async handleTestFlowEvent(@ConnectedSocket() client: Socket, @MessageBody() data: FlowRunWSInput) {
    ...
    const flowResponse = await this.flowResponseService.listen(flowRun._id.toString(), false)
    const flowRunRefreshed = await this.flowRunsService.findOne({
      _id: flowRun.id,
    })
    client.emit(WEBSOCKET_EVENT.TEST_FLOW_FINISHED, flowResponse, flowRunRefreshed)
  }
}
