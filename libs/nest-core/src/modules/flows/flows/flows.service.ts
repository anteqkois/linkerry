...
export class FlowsService {
      ...
      if (operation.type === FlowOperationType.LOCK_AND_PUBLISH) {
        ...
        return this.flowModel
          .findOneAndUpdate(
            ...
            {
              flowVersionId: updatedFlowVersion._id,
            },
            {
              new: true,
            },
  ...
  async createEmpty(projectId: Id, userId: Id) {
    ...
    return (
      await this.flowModel.create({
        _id: flowId,
        projectId,
        flowVersionId: emptyFlowVersion.id,
      })
    ).populate('version')
  }
  ...
  async changeStatus({ newStatus, id, projectId }: UpdateStatusInput) {
    ...
    if (newStatus === FlowStatus.ENABLED && projectFlowsRunningAmount >= currentPlan.config.maximumActiveFlows)
      throw new QuotaError('maximumActiveFlows')
