...
export class AuthController {
  ...
  @Post('signup')
  async signup(
    @BodySchema(signUpInputSchema) body: SignUpInput,
    @Response({ passthrough: true }) res: FastifyReply
  ): Promise<SignUpResponse> {
    const { access_token, user: userRes } = await this.authService.signUp(body);
    const expireDateUnix = +this.configService.get<number>(
      'JWT_ACCES_TOKEN_EXPIRE_SSECONDS',
      3600
    );
  ...
  async login(
    ...
    const expireDateUnix = +this.configService.get(
      'JWT_ACCES_TOKEN_EXPIRE_SSECONDS',
      3600
    );
  ...
  @Post('email/verify')
  async verifyEmail(
    @BodySchema(verifyEmailInputSchema) body: VerifyEmailInput,
